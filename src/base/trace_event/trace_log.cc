...
std::vector<std::unique_ptr<TraceEventFilter>>& GetCategoryGroupFilters() {
  static std::vector<std::unique_ptr<TraceEventFilter>>* filters =
    new std::vector<std::unique_ptr<TraceEventFilter>>();
  return *filters;
}
...
TraceEventHandle TraceLog::AddTraceEventWithThreadIdAndTimestamp(
  ...
  if (thread_id == static_cast<int>(PlatformThread::CurrentId())) {
    ...
    static ThreadLocalPointer<const char>* current_thread_name 
      = new ThreadLocalPointer<const char>();
